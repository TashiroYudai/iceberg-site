...
## コピー全面改訂（2026-07-04・率直/実務トーンへ）
本人ヒアリングで**ポジショニングを明確化**し、全文を書き換え・デプロイ済み（LIVE確認）。
- **狙う相手**＝「何が課題かも分からない」「作業に時間がかかるが、どう直せばいいか分からない」層。
- **主役**＝相手の**課題を見つけ→ツールをつくり→解決する**（＝一番のコア）。
- **ジョブ理論**＝ツール販売ではない。**ツールは手段**、届けるのは**価値**（時間短縮・ミス削減→**金額換算すると非常に大きい**）。
- **流れ**＝ヒアリング→調査（原因特定）→ツール制作→運用しながら**改善し続け価値を伸ばす**。
- 方向性（本人選択）：①価値/ROIは**新セクションを作らず既存に織り込む**（SERVICES導入＝価値の主張、DEPTHで補強）②トーンは**率直・実務的**（詩的メタファーを弱め具体語）③ヒーロー導入は**"わからなさ"に真正面から**。
- 変更箇所：HERO lead／SERVICES eyebrow「解決のツール」+h2「課題に合わせて、ツールをつくる。」+導入p（JTBD）+カード3枚（時間/ミス視点）／DEPTH p（10-90をツール選択に接続）／APPROACH h2「課題を見つけ、ツールにして、育てる。」+strata4層（話を聞く/原因を突き止める/ツールをつくる/使いながら良くし続ける）／CONTACT h2「まずは、話を聞かせてください。」+p／SEO/OGP/JSON-LD/footer tagline「課題を見つけて、ツールで解決する。」に統一。**構造・レイアウト・アニメ・氷山SVGは不変**（テキストのみ）。最新コミット `81429a4`。
...
- **仕上げ**：フッター拡充（ブランド＋タグライン＋`.f-nav`＋著作権2段）。
